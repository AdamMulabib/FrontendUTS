      ...
      <div className="flex flex-wrap justify-center gap-8">
        {speakers.map((s, i) => (
          ...
          >
            ...
            <motion.div
              className="absolute -top-12 left-1/2 -translate-x-1/2 w-24 h-24 rounded-full border-4 border-[#7B1D3F] overflow-hidden bg-pink-100"
              variants={zoomIn}
            >
              <img
                src={s.imageUrl}
        ...
        {faqItems.map((item, i) => (
          ...
          >
            <summary className="flex items-center gap-3 text-gray-700 font-medium text-sm list-none outline-none">
              <ChevronDown
                className="text-[#7B1D3F] group-open:rotate-180 transition-transform"
                size={20}
              />
              {item.title}
            </summary>
